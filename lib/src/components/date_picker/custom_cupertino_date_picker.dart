...
const double _kPickerWidth = 1020.0;
const double _kPickerHeight = 216.0;
const bool _kUseMagnifier = true;
const double _kMagnification = 2.35 / 2.1;
// const double _kDatePickerPadSize = 12.0;
const double _kDatePickerPadSize = 32.0;
// The density of a date picker is different from a generic picker.
// Eyeballed from iOS.
const double _kSqueeze = 1.25;

const TextStyle _kDefaultPickerTextStyle = TextStyle(fontFamily:'Inter', letterSpacing: -0.83);
...
class CustomCupertinoDatePicker extends StatefulWidget {
  ...
  /// changes. When in [CustomCupertinoDatePickerMode.time] mode, the year, month and
  ...
  /// [CustomCupertinoDatePickerMode.date] mode. Defaults to 1.
  ///
    ...
    this.selectionOverlayBuilder,
        ...
        assert(
          (mode != CustomCupertinoDatePickerMode.date &&
              ...
              (minimumYear >= 1 &&
                  (initialDateTime ?? DateTime.now()).year >= minimumYear),
        ...
        assert(
          (mode == CustomCupertinoDatePickerMode.date) || !showDayOfWeek,
  ...
  /// The mode of the date picker as one of [CustomCupertinoDatePickerMode]. Defaults
  /// to [CustomCupertinoDatePickerMode.dateAndTime]. Value cannot change after
  /// initial build.
  ...
  /// Whether to show the day of week alongside the day in [CustomCupertinoDatePickerMode.date] mode.
  ///
  /// Defaults to false.
  final bool showDayOfWeek;

  /// Whether to show the time separator between hour and minute in the time
  /// [CustomCupertinoDatePickerMode.time] and datetime [CustomCupertinoDatePickerMode.dateAndTime]
  /// picker modes.
  ///
  /// Throws an error if set to true in [CustomCupertinoDatePickerMode.date]
  /// and [CustomCupertinoDatePickerMode.monthYear] mode.
  ///
  /// Defaults to false.
    ...
    return texts
        .map(
          (String text) => TextPainter.computeMaxIntrinsicWidth(
            text: TextSpan(
                style: textStyle ?? _themeTextStyle(context), text: text),
...
class _CupertinoDatePickerDateTimeState
  ...
  void didUpdateWidget(CustomCupertinoDatePicker oldWidget) {
    ...
    assert(oldWidget.mode == widget.mode,
      ...
      child: CupertinoPicker.builder(
        ...
        itemBuilder: (BuildContext context, int index) {
          ...
          return itemPositioningBuilder(
              context, Text(dateText, style: _themeTextStyle(context)));
        },
    ...
    return ExcludeSemantics(
      child: CupertinoPicker(
        ...
        children: List<Widget>.generate(1, (int index) {
          return itemPositioningBuilder(
              context, Text(':', style: _themeTextStyle(context)));
        }),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.showTimeSeparator) {
      columnWidths.insert(
          1, _getEstimatedColumnWidth(_PickerColumnType.timeSeparator));
        ...
        LayoutId(
          ...
          child: pickerBuilders[i](
            ...
            (BuildContext context, Widget? child) {
              // "late final" not needed in normal Dart, just use final
...
class _CupertinoDatePickerMonthYearState
  ...
  void _refreshEstimatedColumnWidths() {
    estimatedColumnWidths[_PickerColumnType.month.index] =
      ...
      context,
      false,
      standaloneMonth: widget.mode == CustomCupertinoDatePickerMode.monthYear,
    );
    estimatedColumnWidths[_PickerColumnType.year.index] =
        CustomCupertinoDatePicker._getColumnWidth(
      _PickerColumnType.year,
  ...
  Widget build(BuildContext context) {
    ...
    for (int i = 0; i < columnWidths.length; i++) {
      ...
      double offAxisFraction =
          textDirectionFactor * (firstColumn ? -0.1 : 0.25);
...
class _CupertinoTimerPickerState extends State<CupertinoTimerPicker> {
  ...
  int get textDirectionFactor => textDirection == TextDirection.ltr ? 1 : -1;
  ...
  void _measureLabelMetrics() {
    ...
    textPainter.text =
        TextSpan(text: '$widestNumber$widestNumber', style: textStyle);
