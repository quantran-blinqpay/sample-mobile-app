...
/// the [AppForm] widget as a common ancestor of all of those. Call methods on
/// [AppFormState] to save, reset, or validate each [FormField] that is a
/// descendant of this [AppForm]. To obtain the [AppFormState], you may use [AppForm.of]
/// with a context whose ancestor is the [AppForm], or pass a [GlobalKey] to the
/// [AppForm] constructor and call [GlobalKey.currentState].
///
/// {@tool dartpad}
/// This example shows a [AppForm] with one [TextFormField] to enter an email
/// address and an [ElevatedButton] to submit the form. A [GlobalKey] is used here
/// to identify the [AppForm] and validate input.
///
...
class AppForm extends StatefulWidget {
  ...
  /// Returns the [AppFormState] of the closest [AppForm] widget which encloses the
  ...
  ///  * [PopScope.canPop], which is what [AppForm] delegates to internally.
  final bool? canPop;
...
/// State associated with a [AppForm] widget.
///
/// A [AppFormState] object can be used to [save], [reset], and [validate] every
/// [AppFormField] that is a descendant of the associated [AppForm].
///
/// Typically obtained via [AppForm.of].
class AppFormState extends State<AppForm> {
  int _generation = 0;
  bool _hasInteractedByUser = false;
  final Set<AppFormFieldState<dynamic>> _fields =
      <AppFormFieldState<dynamic>>{};
  ...
  void _fieldDidChange() {
    ...
    _hasInteractedByUser = _fields.any(
        (AppFormFieldState<dynamic> field) => field._hasInteractedByUser.value);
    _forceRebuild();
  }
  ...
  void _register(AppFormFieldState<dynamic> field) {
    _fields.add(field);
  }

  void _unregister(AppFormFieldState<dynamic> field) {
    _fields.remove(field);
  }

  Set<AppFormFieldState<dynamic>> getFields() => _fields;
  ...
  /// Saves every [AppFormField] that is a descendant of this [FFForm].
  void save() {
  ...
  /// Resets every [FormField] that is a descendant of this [AppForm] back to its
  /// [FormField.initialValue].
  ///
  /// The [AppForm.onChanged] callback will be called.
  ///
  /// If the form's [AppForm.autovalidateMode] property is [AutovalidateMode.always],
  ...
  /// Validates every [AppFormField] that is a descendant of this [AppForm], and
  /// returns true if there are no errors.
  ///
  /// The form will rebuild to report the results.
  bool validate({Set<AppFormFieldState<dynamic>>? fields}) {
    _hasInteractedByUser = true;
    _forceRebuild();
    return _validate(fields: fields);
  }

  bool _validate({Set<AppFormFieldState<dynamic>>? fields}) {
    var hasError = false;
  ...
  List<AppFormFieldState> getInvalidFields() {
    return _fields
        .where((element) => element.status == TFStatus.error)
        .toList();
  }
}
...
class _FormScope extends InheritedWidget {
  ...
  /// The [AppForm] associated with this widget.
...
/// otherwise.
///
/// Used by [AppFormField.validator].
typedef FFormFieldValidator<T> = FTextFieldStatus? Function(T? value);

/// Signature for being notified when a form field changes value.
///
/// Used by [AppFormField.onSaved].
typedef FFormFieldSetter<T> = void Function(T? newValue);

/// Signature for building the widget representing the form field.
///
/// Used by [AppFormField.builder].
typedef FormFieldBuilder<T> = Widget Function(AppFormFieldState<T> field);
...
///
/// When used inside a [AppForm], you can use methods on [AppFormState] to query or
/// manipulate the form data as a whole. For example, calling [AppFormState.save]
/// will invoke each [AppFormField]'s [onSaved] callback in turn.
///
/// Use a [GlobalKey] with [AppFormField] if you want to retrieve its current
/// state, for example if you want one form field to depend on another.
///
/// A [AppForm] ancestor is not required. The [AppForm] allows one to
/// save, reset, or validate multiple fields at once. To use without a [AppForm],
/// pass a [GlobalKey] to the constructor and use [GlobalKey.currentState] to
/// save or reset the form field.
///
/// See also:
///
///  * [AppForm], which is the widget that aggregates the form fields.
...
class AppFormField<T> extends StatefulWidget {
  ...
  ///
  /// The returned value is exposed by the [AppFormFieldState.errorText] property.
...
/// The current state of a [AppFormField]. Passed to the [FormFieldBuilder] method
...
class AppFormFieldState<T> extends State<AppFormField<T>>
  ...
  void reset() {
    ...
    AppForm.maybeOf(context)?._fieldDidChange();
  }
  ...
  ///
  /// Triggers the [AppForm.onChanged] callback and, if [AppForm.autovalidateMode] is
  ...
  @override
  void deactivate() {
    AppForm.maybeOf(context)?._unregister(this);
    super.deactivate();
  }
  ...
  Widget build(BuildContext context) {
    ...
    AppForm.maybeOf(context)?._register(this);
